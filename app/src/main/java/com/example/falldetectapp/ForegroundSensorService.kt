...
class ForegroundSensorService : Service(), SensorEventListener {
    ...
    companion object {
        ...
        // Threshold is loaded from assets/deploy_config.json via TFLiteRunner so
        // it stays in sync with whatever model.tflite is shipped. The current
        // model's val-tuned threshold is ~0.45; physics gates (impact, free-fall,
        // post-impact stillness) absorb the false positives that get past it.
    ...
    private fun analyzeCurrentWindow() {
        ...
        if (confidence < tfliteRunner.threshold) {
            return
        }
